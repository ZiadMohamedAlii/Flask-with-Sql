...
class FlaskTest(unittest.TestCase):
    ...
    # home page returns the correct content
    def test_home_content(self):
        tester = app.test_client(self)
        response = tester.get('/')
        self.assertTrue(b'Home Page' in response.data)

    #making sure that adding a new student page returns the correct content
    def test_add_student_page_content(self):
        tester = app.test_client(self)
        response = tester.get('/addstudent')
        self.assertTrue(b'Add New Student' in response.data)

    # making sure that  adding a new student works correctly
    def test_add_student(self):
        tester = app.test_client(self)
        response = tester.post('/addrec', data=dict(name='Ziad Mohamed', address='123 Main St', city='Cairo', pin='12345'), follow_redirects=True)
        self.assertIn(b'Record successfully added!', response.data)

    #  making sure that listing students page returns the correct content
